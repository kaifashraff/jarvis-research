...
- For any live Agent Analytics task, always use the official CLI through `npx @agent-analytics/cli@0.5.5 <command>`.
...
- If a task needs multiple steps, compose the answer from multiple `npx @agent-analytics/cli@0.5.5 ...` commands instead of switching transports.
- If the CLI hits a limitation, stay on the CLI path, explain the limitation, and prefer `npx @agent-analytics/cli@0.5.5 feedback` over inventing a non-CLI workaround.
...
- OpenClaw can launch the official CLI with `npx @agent-analytics/cli@0.5.5`.
...
- Agents should still use the pinned `npx @agent-analytics/cli@0.5.5 ...` form instead of bypassing the CLI.
...
- Use only `npx @agent-analytics/cli@0.5.5 ...` for live queries unless the user explicitly requests API, MCP, or a local binary.
...
- If the task requires manual aggregation across projects, do that aggregation after collecting the data via repeated `npx @agent-analytics/cli@0.5.5 ...` calls.
- Validate project names before `create`: `^[a-zA-Z0-9._-]{1,64}$`

## First-time setup

```bash
npx @agent-analytics/cli@0.5.5 login --detached
npx @agent-analytics/cli@0.5.5 create my-site --domain https://mysite.com
npx @agent-analytics/cli@0.5.5 events my-site --days 7 --limit 20
```
...
```bash
npx @agent-analytics/cli@0.5.5 login --detached
npx @agent-analytics/cli@0.5.5 login --token aak_YOUR_API_KEY
```
...
1. run `npx @agent-analytics/cli@0.5.5 login --detached`
...
npx @agent-analytics/cli@0.5.5 all-sites --period 7d
npx @agent-analytics/cli@0.5.5 stats my-site --days 7
npx @agent-analytics/cli@0.5.5 insights my-site --period 7d
npx @agent-analytics/cli@0.5.5 events my-site --days 7 --limit 20
npx @agent-analytics/cli@0.5.5 breakdown my-site --property path --event page_view --limit 10
npx @agent-analytics/cli@0.5.5 funnel my-site --steps "page_view,signup,purchase"
npx @agent-analytics/cli@0.5.5 retention my-site --period week --cohorts 8
npx @agent-analytics/cli@0.5.5 experiments list my-site
```

If a task needs something outside these common flows, use `npx @agent-analytics/cli@0.5.5 --help` first.
...
1. Run `npx @agent-analytics/cli@0.5.5 projects`
2. For each project, run:

```bash
npx @agent-analytics/cli@0.5.5 query my-site --metrics session_count --filter '[{"field":"timestamp","op":"gte","value":"2026-03-26T12:00:00Z"}]'
```
...
Use `npx @agent-analytics/cli@0.5.5 feedback` when Agent Analytics was confusing, a task took too long, the workflow could be improved, or the agent had to do manual calculations or analysis that Agent Analytics should have handled.
...
```bash
npx @agent-analytics/cli@0.5.5 feedback --message "The agent had to calculate funnel drop-off manually" --project my-site --command "npx @agent-analytics/cli@0.5.5 funnel my-site --steps page_view,signup,purchase"
```
...
1. Run `npx @agent-analytics/cli@0.5.5 create my-site --domain https://mysite.com`
2. Copy the returned snippet into the page before `</body>`
3. Deploy
4. Verify with `npx @agent-analytics/cli@0.5.5 events my-site --days 7 --limit 20`
...
`npx @agent-analytics/cli@0.5.5 query` exists for advanced reporting, but it should be used carefully because `--filter` accepts JSON.

- Use fixed commands first.
- If `query` is necessary, check `npx @agent-analytics/cli@0.5.5 --help` first.
- Do not pass raw user text directly into `--filter`.
- The only valid CLI shape is `npx @agent-analytics/cli@0.5.5 query <project> ...`. Do not use `--project`.
...
2. Run `npx @agent-analytics/cli@0.5.5 properties <project>` to inspect event names and property keys first.
3. Use `npx @agent-analytics/cli@0.5.5 query <project> --filter ...` for property-filtered counts.
4. Use `npx @agent-analytics/cli@0.5.5 events <project>` only to validate ambiguous payloads or missing properties.
5. Use `npx @agent-analytics/cli@0.5.5 feedback` if the requested slice depends on unsupported grouping or derived reporting.
...
```bash
npx @agent-analytics/cli@0.5.5 properties my-site
npx @agent-analytics/cli@0.5.5 query my-site --metrics event_count --filter '[{"field":"event","op":"eq","value":"page_view"},{"field":"properties.first_utm_source","op":"eq","value":"reddit"}]' --days 30
```
...
```bash
npx @agent-analytics/cli@0.5.5 experiments list my-site
npx @agent-analytics/cli@0.5.5 experiments create my-site --name signup_cta --variants control,new_cta --goal signup
```
